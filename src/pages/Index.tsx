...
function ToolCard({ tool, index }: { tool: typeof AI_TOOLS[number]; index: number }) {
  ...
  return (
    <motion.a
      ...
      className="group block rounded-xl bg-card border border-border p-5 transition-all duration-300 hover:bg-card-hover hover:scale-[1.03] hover:shadow-[0_0_30px_hsl(var(--neon-glow)/0.15)] hover:border-primary/30 cursor-pointer"
    >
      <div className="flex items-start justify-between mb-3">
        <span className="text-3xl">{tool.logo}</span>
        <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${priceBadgeStyles[tool.priceType]}`}>
          {priceBadgeLabels[tool.priceType]}
        </span>
      </div>
      <h3 className="text-foreground font-bold text-base mb-1 group-hover:text-primary transition-colors">
        {tool.name}
      </h3>
      <p className="text-muted-foreground text-xs mb-3 leading-relaxed">{tool.description}</p>
      <p className="text-primary/60 text-[10px] mb-3">{tool.studentOffer}</p>
      <div className="flex items-center gap-1.5 text-primary text-xs font-bold group-hover:gap-2.5 transition-all">
      ...
      <section className="relative h-screen overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center justify-center h-full text-center px-4">
          <motion.h1
            ...
            className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-extrabold text-foreground tracking-tight leading-[0.95] mb-4"
          >
            100+ FREE AI TOOLS
            <br />
          ...
          <motion.a
            ...
            className="mt-8 bg-primary text-primary-foreground font-bold px-8 py-3 rounded-full text-sm hover:scale-105 transition-transform inline-flex items-center gap-2"
          >
